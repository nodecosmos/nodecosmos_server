...
impl Callbacks for FlowStep {
    ...
    async fn before_insert(&mut self, session: &CachingSession, data: &RequestData) -> Result<(), NodecosmosError> {
        ...
        if self.is_branched() {
            let flow =
                Flow::find_by_node_id_and_branch_id_and_id(session, self.node_id, self.branch_id, self.flow_id).await?;
    ...
    async fn before_delete(&mut self, _session: &CachingSession, data: &RequestData) -> Result<(), NodecosmosError> {
...
impl Callbacks for UpdateNodeIdsFlowStep {
    ...
    async fn after_update(&mut self, session: &CachingSession, data: &RequestData) -> Result<(), NodecosmosError> {
