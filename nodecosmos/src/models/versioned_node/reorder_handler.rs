...
use crate::models::versioned_node::create::{NewDescendantVersionById, NodeChange};
...
impl VersionReorderHandler {
    ...
    pub async fn run(&self) -> Result<(), NodecosmosError> {
        ...
        let update_ancestors = !self.parent_changed;
        let new_node_version = self.create_new_node_version(update_ancestors).await?;
    ...
    async fn create_new_node_version(&self, update_ancestors: bool) -> Result<VersionedNode, NodecosmosError> {
        let changes = vec![
            NodeChange::OrderIndex(self.new_order_index),
            NodeChange::Parent(self.new_parent_id),
            NodeChange::Ancestors(self.removed_ancestor_ids.clone(), self.added_ancestor_ids.clone()),
        ];
    ...
    async fn create_new_version_for_descendants(&self) -> Result<NewDescendantVersionById, NodecosmosError> {
        let mut descendant_version_by_id = HashMap::new();
        let changes = vec![NodeChange::Ancestors(
            self.removed_ancestor_ids.clone(),
            self.added_ancestor_ids.clone(),
        )];
        ...
        for id_chunk in self.descendant_ids.chunks(MAX_PARALLEL_REQUESTS) {
            ...
            for result in results {
                match result {
                    ...
                    Err(err) => {
                        log_fatal(format!("create_new_version_for_descendants: {:?}", err));
                    }
                }
            }
        }
    ...
    async fn create_new_version_for_removed_ancestors(&self) -> Result<(), NodecosmosError> {
        ...
        let changes = vec![NodeChange::Descendants(removed_descendant_ids.clone(), None)];
        ...
        for id_chunk in self.removed_ancestor_ids.chunks(MAX_PARALLEL_REQUESTS) {
            ...
            for result in results {
                match result {
                    Ok(_) => {}
                    Err(err) => {
                        log_fatal(format!("create_new_version_for_removed_ancestors: {:?}", err));
                    }
                }
            }
        }
    ...
    async fn create_new_version_for_added_ancestors(
        ...
        let changes = vec![NodeChange::Descendants(
            self.descendant_ids.clone(),
            Some(dv_by_id.clone()),
        )];
        ...
        for id_chunk in self.added_ancestor_ids.chunks(MAX_PARALLEL_REQUESTS) {
            ...
            for result in results {
                match result {
                    Ok(_) => {}
                    Err(err) => {
                        log_fatal(format!("create_new_version_for_added_ancestors: {:?}", err));
                    }
                }
            }
        }
