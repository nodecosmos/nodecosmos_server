...
use crate::models::traits::RootId;
...
impl ResourceLocker {
    ...
    pub async fn validate_root_unlocked<M: RootId>(&self, model: &M, retry: bool) -> Result<(), NodecosmosError> {
        if self.is_resource_locked(&model.root_id().to_string()).await? {
            if retry {
